...
function NPC_state_scripts(){

}

//this is much like the player collisions and movement. Might need to break into piece, but works for now. Easier to manage since NPC behavior is programmable
function NPC_collisions_movement(){
	//X collision
		var _subPixel = .5;
...
x += xspd; //make sure this happens after collision code
	...
	//Rest/Prepare Jump Variables
		 if onGround{
			...
			//if npc is in the air make sure they can't do an extra jump
			coyoteJumpTimer--;
...
//Y Collision
		...
		/* // you need to replace the conditions of this before implementing this
...
function NPC_attack_command(_nextState,_attackType,_comboFrame){
	...
	if (attackActionStart && onGround && image_index>=_comboFrame){
		state = _nextState;
		image_index=0;
		myHitBox = instance_create_depth(x,y,depth,oNPCHitBox,{
